...
class GroceryStoreSimulation:
    ...
    def run(self, file: TextIO) -> Dict[str, Any]:
        ...
        while not self._events.is_empty():
            ...
            if e.timestamp > total_time:
                total_time = e.timestamp
            if type(e) is CustomerArrival:
                cus_dict[e.customer] = e.timestamp
            elif type(e) is CheckoutCompleted:
                cus_dict[e.customer] = e.timestamp - \
                                            cus_dict[e.customer.name]
            e_lst = e.do(self._store)
            for i in e_lst:
                self._events.add(i)

        max_wait = 0
        for customer in cus_dict:
            if cus_dict[customer] > max_wait:
                max_wait = cus_dict[customer]

        stats = {
            'num_customers': len(cus_dict),
