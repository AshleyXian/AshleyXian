...
def mergesort3(lst: List) -> List:
    ...
    if len(lst) == 2:
        ...
        divider = len(lst) // 3
        first = mergesort3(lst[:divider])
        second = mergesort3(lst[divider:divider * 2])
        third = mergesort3(lst[divider * 2:])
...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    ind3 = 0
    merged = []
    while ind1 < len(lst1) and ind2 < len(lst2) and ind3 < len(lst3):
        if (lst1[ind1] <= lst2[ind2]) and (lst1[ind1] <= lst3[ind3]):
            merged.append(lst1[ind1])
            ind1 += 1
        elif (lst2[ind2] <= lst1[ind1]) and (lst2[ind2] <= lst3[ind3]):
            merged.append(lst2[ind2])
            ind2 += 1
        elif (lst3[ind3] <= lst1[ind1]) and (lst3[ind3] <= lst2[ind2]):
            merged.append(lst3[ind3])
            ind3 += 1
    if ind1 == len(lst1):
        merged.extend(_merge(lst2[ind2:], lst3[ind3:]))
    elif ind2 == len(lst2):
        merged.extend(_merge(lst1[ind1:], lst3[ind3:]))
    else:
        merged.extend(_merge(lst1[ind1:], lst2[ind2:]))
    return merged
