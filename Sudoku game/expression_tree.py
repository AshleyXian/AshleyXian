...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        if self.is_empty():
            ...
            if type(self._root) is int:
                return self._root
            elif type(self._root) is str and \
                    self._root != OP_MULTIPLY and self._root != OP_ADD:
            ...
            for subtree in self._subtrees:
                if result is None:
                    ...
                    if self._root == OP_MULTIPLY:
                        result = result * subtree.eval(lookup)
        ...
        if self.is_empty():
            ...
            for subtree in self._subtrees:
                if not subtree._subtrees:
                    if result == '':
                        result = f'({subtree.__str__()}'
                    else:
                        result = result + f' {self._root} ' + \
                                 f'{subtree.__str__()}'
                else:
                    if result == '':
                        result = '(' + subtree.__str__()
                    else:
                        result = result + f' {self._root} ' + subtree.__str__()
            return result + ')'
    ...
    def substitute(self, from_to: Dict[Union[str, int],
        ...
        (2 + (2 + 1))
        """
        if self._subtrees == [] and self._root in from_to:
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        if self.is_empty():
            pass
        elif not self._subtrees and type(self._root) is str and \
                self._root != OP_MULTIPLY and self._root != OP_ADD:
            lookup[self._root] = 0
        else:
            for subtree in self._subtrees:
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    True
    >>> ex2 = [['+'], [3, '*', 'a', '+'], ['a', 'b'], [5, 'c']]
    >>> tree2 = construct_from_list(ex2)
    >>> print(tree2)
    """
    if values[0][0] != OP_MULTIPLY and values[0][0] != OP_ADD:
        ...
        while i < len(values):
            ...
            for item in values[i]:
                if item != OP_MULTIPLY and item != OP_ADD:
                    result.append(ExprTree(item, []))
                else:
                    cur = ExprTree(item, [])
                    cur.append(construct_from_list([[item], values[i+j]]))
                    result.append(cur)
                    j = j + 1
            i = i + j
